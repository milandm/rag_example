...
from text_bot.utils import (remove_quotes,
                            ...
                            split_string_using_predefined_delimiters)
...
from text_bot.nlp_model.entity_linking.prompt_template_creator import \
...
class PromptCreator:
    ...
    def enrich_dnb_data(self, screened_person_dnb: str):
        title_extract_prompt = self.prompt_template_creator.get_enrich_dnb_data_prompt(screened_person_dnb)
        document_title_openai_response = self.model.send_prompt(SYSTEM_MSG_COMPLIANCE, title_extract_prompt)
    ...
    def get_persons_list(self, screened_person: str):
        title_extract_prompt = self.prompt_template_creator.get_screened_person_additional_data_prompt(screened_person)
        document_title_openai_response = self.model.send_prompt(SYSTEM_MSG_COMPLIANCE, title_extract_prompt)
        document_title_content = document_title_openai_response.choices[0].message.content
        document_title = extract_single_value_openai_content(document_title_content, TITLE_EXTRACT_KEY)
        return document_title
    ...
    def get_inconsistencies(self, screened_person: str, potential_matches: List[Dict]):
        title_extract_prompt = self.prompt_template_creator.get_personal_data_inconsistencies_prompt(screened_person, str(potential_matches))
        document_title_openai_response = self.model.send_prompt(SYSTEM_MSG_COMPLIANCE, title_extract_prompt)
        document_title_content = document_title_openai_response.choices[0].message.content
        document_title = extract_single_value_openai_content(document_title_content, TITLE_EXTRACT_KEY)
        return document_title
    ...
    def compare_persons(self, screened_person: str, potential_matches: List[Dict]):
        title_extract_prompt = self.prompt_template_creator.get_screened_person_matches_prompt(screened_person, str(potential_matches))
        document_title_openai_response = self.model.send_prompt(SYSTEM_MSG_COMPLIANCE, title_extract_prompt)
        document_title_content = document_title_openai_response.choices[0].message.content
        document_title = extract_single_value_openai_content(document_title_content, TITLE_EXTRACT_KEY)
        return document_title
    ...
    def search_more_individuals(self, screened_person: dict):
        title_extract_prompt = self.prompt_template_creator.get_screened_search_more_prompt(str(screened_person))
        document_title_openai_response = self.model.send_prompt(SYSTEM_MSG_COMPLIANCE, title_extract_prompt)
        document_title_content = document_title_openai_response.choices[0].message.content
        document_title = extract_single_value_openai_content(document_title_content, TITLE_EXTRACT_KEY)
        return document_title
